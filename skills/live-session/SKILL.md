...
- **Narrer les actions adverses dans l'ordre de `streets[].actions`** (ou, action par action, dans l'ordre des échos `applied_to` renvoyés par `scripts/pc apply`) — **jamais dans l'ordre de lecture du rendu ASCII**. Le rendu est un plan de table (des sièges disposés dans l'espace), pas une séquence de parole : le lire de haut en bas produit un ordre qui n'a aucune raison de coïncider avec l'ordre réel, et qui n'en diverge parfois que d'un cran — donc invisible à la relecture. Constat de session (bug de narration) : les actions adverses ont été racontées dans un ordre différent de celui réellement appliqué au moteur, avec des montants pourtant justes. **Un ordre narré divergent est un bug même quand les montants sont bons** — l'utilisateur lit une séquence de parole (qui a ouvert, qui a réagi à qui) et bâtit sa lecture des profils dessus ; `pc apply` renvoie désormais `applied_to` (siège, position, action, montant) précisément pour que cet ordre soit vérifiable après coup, l'état résultant seul ne disant que qui parle **ensuite**.
- **Ne jamais retoucher `hand.json` à la main pour contourner un script qui refuse** (statuts de sièges remis à `active`, nœud de rue écrit directement, `to_act` réécrit). Un refus est un état à corriger ou un bug d'outil à signaler, jamais une étape à sauter : l'état trafiqué passe ensuite toutes les validations sans plus rien signaler. C'est ce qu'il a fallu faire en session pour dérouler un all-in callé — ce cas précis est corrigé (voir « Runout » plus bas), la règle reste. Seule exception : les cas que `new_hand.py` nomme explicitement comme hors de son scope (side pots multiway, table rétrécie par un bust) — l'ajustement manuel y est la marche à suivre documentée, et se signale à l'utilisateur.
...
`advance_street.py` : ouvre la rue suivante (preflop→flop attend 3 cartes, flop→turn et turn→river en attendent 1). Refuse si l'action de la rue courante n'est pas close (il manque une décision), si le nombre de cartes ne correspond pas, si une carte est déjà connue dans la main, ou si on est déjà à la river. Réécrit `hand.json` en place et pointe `to_act` sur le bon siège (ordre postflop : la SB en premier, le Bouton en dernier), ou sur `null` s'il n'y a plus de siège à qui donner la parole (runout — voir ci-dessous).
...
Dès qu'un tapis est **callé**, il n'y a plus de décision à prendre — seules des cartes restent à distribuer. `advance_street.py` ouvre alors les rues suivantes **sans action à enregistrer** (c'est normal, pas une rue « non close »), et `pc state`/`pc render`/`pc assert-state` répondent `runout: true`. Selon les tapis, `to_act` vaut `null` (tous les sièges en lice sont all-in) ou pointe encore sur le payeur le plus profond, resté `active` : les deux sont des états valides, et le booléen `runout` est ce qui les décrit, pas `to_act`.
...
Pendant un runout, `pc apply`, `pc brief` et `pc budget` **refusent** (code non nul) : il n'y a aucune décision à conseiller ni à appliquer. Le rendu de chaque rue reste dû (`pc render`), comme pour n'importe quelle transition — et une rue de runout narrée sans `advance_street.py` est exactement le même bug que partout ailleurs.
