...
Reconstruire l'état de la main en `hand.json` (voir le schéma dans `docs/brief/references/02-architecture-v2.md` — table, seats, streets, to_act), puis à chaque point de décision qui intéresse l'utilisateur :

```bash
pc brief --hand hand.json [--villain-archetype X]
```
...
5. **Pour visualiser une main complexe/multiway** si ça aide à la clarté : `pc render --hand hand.json` (pas obligatoire pour une main simple en heads-up).
