...
from .state import (STREETS, HandState, StateError, derive, is_runout,
                     n_behind as derive_n_behind,
...
def _require_hero_to_act(state: HandState) -> None:
    ...
    # Le runout couvre aussi le cas où le héros est resté ACTIVE : payeur le
    # plus profond d'un tapis, il ne peut ni suivre (rien à suivre) ni miser
    # (personne pour payer) -- `to_act` pointe pourtant encore sur lui, et
    # sans cette garde `pc brief` déroulait ses gates jusqu'à un verdict sur
    # une décision qui n'existe pas.
    if is_runout(state) or state.to_act is None:
        raise StateError(
            "pc brief conseille une décision, or il n'y en a plus à prendre : l'all-in est "
            "déjà callé (runout) — dérouler le board avec advance_street.py, puis résoudre "
            "par pc showdown"
        )
